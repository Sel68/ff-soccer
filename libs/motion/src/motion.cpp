...
Motion::Profile1D Motion::planProfile(double distance, MotionConstraints constraints, double v0) {
    ...
    if (distance > p.distToMax + p.distToStop) {
        ...
        p.maxSpeed = std::sqrt((distance + (p.v0 * p.v0) / (2.0 * p.maxAccel)) / (1.0 / (2.0 * p.maxAccel) + 1.0 / (2.0 * p.maxDecel)));
        if (p.maxSpeed < p.v0) {
            p.maxSpeed = p.v0;
            p.timeToMax = 0.0;
            p.distToMax = 0.0;
...
Motion::GeneratedProfile Motion::generateProfile(Point start, Point end, double current_theta, VelocityState initial_velocity) {
    ...
    profile.thetaProfile = planProfile(totalDistanceTheta, thetaDirectionConstraints, initial_velocity.vtheta);
    ...
    profile.totalTime = std::max({profile.xProfile.totalTime, profile.yProfile.totalTime, profile.thetaProfile.totalTime});
...
Motion::VelocityState Motion::getVelocityState(const GeneratedProfile& profile, double t) {
    ...
    KinematicState thetaState = calculateKinematicState(t, profile.thetaProfile, profile.totalDistanceTheta);
...
Motion::KinematicState Motion::calculateKinematicState(
    double t, const Profile1D& profile, double totalDistance) {
