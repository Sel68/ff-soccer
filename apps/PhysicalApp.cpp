...
class Host {
 public:
  ...
  void LoopStep() {
    ...
    std::vector<CameraValues> myVect = camera_comm.GetCameraValues();

    soccer.ProcessInput(dt, myVect[0].x, myVect[0].y, myVect[0].orientation);
    soccer.Update(dt);

    RobotCommands robot_cmds = PrepareRobotCommands(soccer, myVect);
