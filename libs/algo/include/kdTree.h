...
class KDTree {
 public:
  ...
  /** @brief Inserts a point into the k-d tree.
   */
  void insert(const PointT& point) {
    points_.push_back(point);
    int new_idx = (int)points_.size() - 1;
    root_ = insertRecursive(root_, new_idx, 0);
  }
 ...
 private:
  ...
  /** @brief Inserts a node recursively.
   */
  Node* insertRecursive(Node* node, int idx, int depth) {
    if (node == nullptr) {
      Node* new_node = new Node();
      new_node->idx = idx;
      new_node->axis = depth % PointT::DIM;
      return new_node;
    }

    const int axis = node->axis;
    if (points_[idx][axis] < points_[node->idx][axis]) {
      node->next[0] = insertRecursive(node->next[0], idx, depth + 1);
    } else {
      node->next[1] = insertRecursive(node->next[1], idx, depth + 1);
    }
  ...
  static double distance(const PointT& p, const PointT& q) {
    double dist = 0;
    for (size_t i = 0; i < PointT::DIM; i++) dist += (p[i] - q[i]) * (p[i] - q[i]);
    return sqrt(dist);
  }
  ...
  /** @brief Searches neighbors within radius.
   */
  void radiusSearchRecursive(const PointT& query, const Node* node, std::vector<int>& indices,
                             double radius) const {
    if (node == nullptr) return;
