...
protected:
    std::vector<WheelConfig> wheel_configs;
    double max_spin;
    ...
    void SetUp() override {
        ...
        wheel_configs.push_back({R, 0.0, PI / 2.0, r});           // Front (on X axis)
        wheel_configs.push_back({R, PI / 2.0, PI, r});            // Left (on Y axis)
        wheel_configs.push_back({R, PI, 3.0 * PI / 2.0, r});      // Back (on -X axis)
        wheel_configs.push_back({R, 3.0 * PI / 2.0, 0.0, r});     // Right (on -Y axis)
    }
};
...
TEST_F(OmniKinematicsTest, ScaleLimits) {
    ...
    ChassisVelocity target = {1.0, 0.0, 0.0}; // Move +1.0 m/s in X (would normally require 20 rad/s)
